...
export class GoalsComponent implements OnDestroy {
  ...
  addGoalString() {
    this.goals.push({  Description: '', Count: 0, UserId: '', ExerciseId: '', showDropdown: true } as any);
  }
  loadExercises() {
  ...
  GetGoalsByUserIdAsync(): void {
    this.goalService.GetGoalsByUserIdAsync().pipe(
      takeUntil(this.destroy$)
    ).subscribe(
      (goals: GoalModel[]) => {
        this.goals = goals;
  ...
  addGoal() {
    this.authService.getUserId().pipe(
      switchMap(userId => {
        const selectedDescription: Exercise | null | undefined = this.goalForm.value.description as Exercise | null | undefined;
        if (!selectedDescription || typeof selectedDescription !== 'object' || !('name' in selectedDescription) || !('id' in selectedDescription)) {
  ...
  showStatistic(goalId: string) {
    this.statisticService.getStatisticById(goalId).pipe(
      takeUntil(this.destroy$)
    ).subscribe(
